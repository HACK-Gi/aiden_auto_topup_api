...
# អាន .env ហើយឲ្យតម្លៃពី .env ជាន់លើអថេរចាស់ៗក្នុងប្រព័ន្ធ
load_dotenv(override=True)

app = Flask(__name__)
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'dev-secret')
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'postgresql://postgres:Mengly2013!!@db.hglqfentlmowlqgftqqx.supabase.co:5432/postgres')
...
def populate_services():
    if Service.query.first() is None:
        for game, product, supplier, cmd, needs_server in SERVICES_DATA:
            selling = apply_markup(float(supplier))
            s = Service(
                game=game, product_name=product,
                supplier_price=float(supplier), selling_price=selling,
...
def register():
    ...
    if request.method == 'POST':
        ...
        db.session.add(user)
        db.session.commit()
        send_telegram(ADMIN_CHAT_ID, f"🆕 អ្នកប្រើថ្មី៖ <b>{username}</b>")
        login_user(user)
...
def deposit():
    if request.method == 'POST':
        ...
        flash('សំណើតម្កល់ប្រាក់បានដាក់ស្នើ។ សូមរង់ចាំការយល់ព្រម។', 'info')
        return redirect(url_for('dashboard'))
...
def generate_api_key():
    if current_user.api_key:
        flash('អ្នកមាន API Key រួចហើយ។ អាច Reset បាន។', 'warning')
        return redirect(url_for('dashboard'))
...
def api_order():
    ...
    if not user or not user.is_active:
        return jsonify({"status":"error","message":"Invalid API key or banned"}), 401
    ...
    if game in ['mg', 'mc'] and not server_id:
        return jsonify({"status":"error","message":"Server ID required"}), 400
